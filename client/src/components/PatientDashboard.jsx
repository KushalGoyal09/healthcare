...
const PatientDashboard = () => {
    ...
    const [currentVisit, setCurrentVisit] = useState({ _id: null, doctor: { name: null } });
    const [date, setDate] = useState(null);

    const fetchAppointments = async () => {
        try {
            const { data } = await axios.post(`${backendBaseUrl}/api/dashboard/patient`, {
                ...
                <div key={uniqueDate}>
                    ...
                    {appointments.filter(appointment => new Date(appointment.slot.startTime).toDateString() === uniqueDate).map(filteredAppointment => (
                        <Slot
                            ...
                            onClick={joinMeet(filteredAppointment._id)}
                            name={filteredAppointment.doctor.name}
